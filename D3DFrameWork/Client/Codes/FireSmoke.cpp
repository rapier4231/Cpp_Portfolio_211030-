...
CFireSmoke::CFireSmoke(LPDIRECT3DDEVICE9 _pDevice)
	: CGameObject(_pDevice)
{

}

CFireSmoke::CFireSmoke(const CFireSmoke & rhs)
	: CGameObject(rhs)
{

}

HRESULT CFireSmoke::Ready_GameObject_Prototype()
{
...
HRESULT CFireSmoke::Ready_GameObject(void * _pArg/* = nullptr*/)
{
...
_int CFireSmoke::Update_GameObject(const _double & _dTimeDelta)
{
	if (0.f > m_fAlpha)
		Set_Dead();
...
_int CFireSmoke::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
void CFireSmoke::Render_GameObject()
{
	if (!m_bUpdated)
		return;
...
CFireSmoke * CFireSmoke::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CFireSmoke*	pInstance = new CFireSmoke(_pDevice);
...
CGameObject * CFireSmoke::Clone(void * _pArg/* = nullptr*/)
{
	CFireSmoke*	pClone = new CFireSmoke(*this);
...
void CFireSmoke::Free()
{
	CGameObject::Free();
}

HRESULT CFireSmoke::Add_Component(void)
{
	CComponent*		pComponent = nullptr;

	pComponent = m_pBufferCom = dynamic_cast<CVIBuffer_FireSmokeParticleTex*>(Engine::Clone_ComponentPrototype(L"Proto_Com_VIBuffer_FireSmokeParticleTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(L"Com_Buffer", pComponent);

	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_ComponentPrototype(L"Proto_Texture_FireSmoke"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
HRESULT CFireSmoke::SetUp_ConstantTable(LPD3DXEFFECT & _pEffect)
{
