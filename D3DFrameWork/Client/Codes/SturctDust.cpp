...
CStructDust::CStructDust(LPDIRECT3DDEVICE9 _pDevice)
	: CGameObject(_pDevice)
{

}

CStructDust::CStructDust(const CStructDust & rhs)
	: CGameObject(rhs)
{

}

HRESULT CStructDust::Ready_GameObject_Prototype()
{
...
HRESULT CStructDust::Ready_GameObject(void * _pArg/* = nullptr*/)
{
...
_int CStructDust::Update_GameObject(const _double & _dTimeDelta)
{
	if (0.f > m_fAlpha)
		Set_Dead();
	...
	//if(8 < m_dCount)
		m_fAlpha -= (_float)_dTimeDelta /* 1.f / (m_iTextureX * m_iTextureY)*/;
...
_int CStructDust::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
void CStructDust::Render_GameObject()
{
	if (!m_bUpdated)
		return;
...
CStructDust * CStructDust::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CStructDust*	pInstance = new CStructDust(_pDevice);
...
CGameObject * CStructDust::Clone(void * _pArg/* = nullptr*/)
{
	CStructDust*	pClone = new CStructDust(*this);
...
void CStructDust::Free()
{
	CGameObject::Free();
}

HRESULT CStructDust::Add_Component(void)
{
	CComponent*		pComponent = nullptr;

	pComponent = m_pBufferCom = dynamic_cast<CVIBuffer_StructDustParticleTex*>(Engine::Clone_ComponentPrototype(L"Proto_Com_VIBuffer_StructDustParticleTex"));
...
HRESULT CStructDust::SetUp_ConstantTable(LPD3DXEFFECT & _pEffect)
{
