...
//#define STATbar_EXP	10 원래 주석이였음
...
HRESULT CStat_UI_Bar::Set_Screen_UI()
{
	...
	case Engine::STATbar_Food:
		...
		break;
	case Engine::STATbar_Water:
		m_v4PlusEdge = stURI.v4Water_Plus_EdgeSize[EScrnResolution];
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Water_Bar_EdgeSize[EScrnResolution];

		break;
	case Engine::STATbar_Weight:
	...
	case Engine::STATbar_Speed:
		...
		break;
	case Engine::STATbar_Engram:
		m_v4PlusEdge = stURI.v4Engram_Plus_EdgeSize[EScrnResolution];
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Engram_Bar_EdgeSize[EScrnResolution];

		break;
	case Engine::STATbar_Endure:
		m_v4PlusEdge = stURI.v4Endure_Plus_EdgeSize[EScrnResolution];
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Endure_Bar_EdgeSize[EScrnResolution];

		break;
	case Engine::STATbar_Paralysis:
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Paralysis_Bar_EdgeSize[EScrnResolution];
		m_fMaxPlayerStat = ParalysisMaxStat;
		break;

	case /*Engine::*/STATbar_EXP:
...
CStat_UI_Bar::CStat_UI_Bar(LPDIRECT3DDEVICE9 _pDevice)
	: COrthoUI(_pDevice)
{
}

CStat_UI_Bar::CStat_UI_Bar(const CStat_UI_Bar & rhs)
	: COrthoUI(rhs)
{
}

HRESULT CStat_UI_Bar::Ready_GameObject_Prototype()
{
...
HRESULT CStat_UI_Bar::Ready_GameObject(void * _pArg/* = nullptr*/)
{
	...
	case Engine::STATbar_Food:
		...
		break;
	case Engine::STATbar_Water:
		wstrStatType += L"SummaryStat_Water";
		m_v4PlusEdge = stURI.v4Water_Plus_EdgeSize[EScrnResolution];
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Water_Bar_EdgeSize[EScrnResolution];
	...
	case Engine::STATbar_Speed:
		...
		break;
	case Engine::STATbar_Engram:
		wstrStatType += L"CraftSpeed";
		m_v4PlusEdge = stURI.v4Engram_Plus_EdgeSize[EScrnResolution];
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Engram_Bar_EdgeSize[EScrnResolution];

		break;
	case Engine::STATbar_Endure:
		wstrStatType += L"Fortitude";
		m_v4PlusEdge = stURI.v4Endure_Plus_EdgeSize[EScrnResolution];
		m_tORTHOUI_INFO.v4EdgeSize = stURI.v4Endure_Bar_EdgeSize[EScrnResolution];
...
_int CStat_UI_Bar::Update_GameObject(const _double & _dTimeDelta)
{
...
_int CStat_UI_Bar::LateUpdate_GameObject(const _double & _dTimeDelta)
{
	COrthoUI::LateUpdate_GameObject(_dTimeDelta);
	if (m_tORTHOUI_INFO.bRender)
	{
		if (STATbar_Paralysis != m_eStatType)
		{
			if (0 < m_pPlayer->Get_Stat(CUnit::STAT_STATPOINT))
				m_bCanRiseStat = true;
			else
		...
		case Engine::STATbar_Atk:
			if (!m_fMaxPlayerStat)
				m_fMaxPlayerStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_ATTACK);
			fNowStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_ATTACK);
			break;
		case Engine::STATbar_Speed:
			if (!m_fMaxPlayerStat)
				m_fMaxPlayerStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_SPEED);
			fNowStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_SPEED);
			break;
		case Engine::STATbar_Engram:
			if (!m_fMaxPlayerStat)
				m_fMaxPlayerStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_ENGRAM);
			fNowStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_ENGRAM);
			break;
		case Engine::STATbar_Endure:
			if (!m_fMaxPlayerStat)
				m_fMaxPlayerStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_ENDURE);
			fNowStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_ENDURE);
			break;
		case Engine::STATbar_Paralysis:
			fNowStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_PARALYSIS);
			break;
		case Engine::STATbar_EXP:
			m_fMaxPlayerStat = (_float)static_cast<CPlayer*>(m_pPlayer)->Get_NextExp(); //<<>>
			fNowStat = (_float)m_pPlayer->Get_Stat(CUnit::STAT_EXP);
			break;
		case Engine::STATbar_End:
			break;
		default:
			break;
		}
		...
		if (STATbar_Atk <= m_eStatType && STATbar_EXP != m_eStatType)
...
void CStat_UI_Bar::Render_GameObject(_mat _matOrtho)
{
	if (!m_tORTHOUI_INFO.bRender)
		return;
...
HRESULT CStat_UI_Bar::Add_Component(void)
{
	CComponent*		pComponent = nullptr;
...
HRESULT CStat_UI_Bar::SetUp_ConstantTable(LPD3DXEFFECT & _pEffect, _mat _matOrtho, _int _bTwo)
{
	STStat_UI_BarURI stURI;
	...
	else if (2 == _bTwo)
	{
		...
		else if ((m_tORTHOUI_INFO.uchSelectUI & MouseOnUI)&& m_pPlayer->Get_Stat(CUnit::STAT_STATPOINT)) //마우스가 올라왔으면
...
void CStat_UI_Bar::Render_Font(_mat _matOrtho)
{
	STStat_UI_BarURI stURI;
	...
	case Engine::STATbar_Water:
		...
		wstrNum += L".";
		wstrNum += to_wstring(((_int)(m_fMaxPlayerStat * 10.f)) - (((_int)m_fMaxPlayerStat) * 10));
		Engine::Render_Font(L"Font_ItemGuide", wstrNum, &_vec2(m_tORTHOUI_INFO.fPosX + stURI.fFont_Water_Stat_IntervalPosX[EScrnResolution], m_tORTHOUI_INFO.fPosY - stURI.fFont_Water_Stat_IntervalPosY[EScrnResolution]), StatFontColor, 1);
		wstrNum = L"물";
		break;
	case Engine::STATbar_Weight:
		wstrNum += to_wstring((_int)m_fNowPlayerStat);
	...
	case Engine::STATbar_Engram:
		...
		wstrNum += to_wstring(((_int)(m_fPerPlayerStat * 10.f)) - (((_int)m_fPerPlayerStat) * 10));
		wstrNum += L"%";
		Engine::Render_Font(L"Font_ItemGuide", wstrNum, &_vec2(m_tORTHOUI_INFO.fPosX + stURI.fFont_Engram_Stat_IntervalPosX[EScrnResolution], m_tORTHOUI_INFO.fPosY - stURI.fFont_Engram_Stat_IntervalPosY[EScrnResolution]), StatFontColor, 1);
		wstrNum = L"제작 기술";
		break;
	case Engine::STATbar_Endure:
		wstrNum += to_wstring((_int)m_fNowPlayerStat);
		Engine::Render_Font(L"Font_ItemGuide", wstrNum, &_vec2(m_tORTHOUI_INFO.fPosX + stURI.fFont_Endure_Stat_IntervalPosX[EScrnResolution], m_tORTHOUI_INFO.fPosY - stURI.fFont_Endure_Stat_IntervalPosY[EScrnResolution]), StatFontColor, 1);
		wstrNum = L"인내";
		break;
	case Engine::STATbar_Paralysis:
		wstrNum += to_wstring((_int)m_fNowPlayerStat);
	...
	case Engine::STATbar_EXP:
		wstrNum = L"남은 포인트 ";
		wstrNum += to_wstring((_int)m_pPlayer->Get_Stat(CUnit::STAT_STATPOINT));
	...
	else if (STATbar_Speed > m_eStatType)
	...
	else if (STATbar_EXP > m_eStatType)
...
void CStat_UI_Bar::PlusMouseCheck(const _double & _dTimeDelta)
{
			...
			if (FinalStatUpCoolTime < m_fStatUp_CoolTime) //누르면 0되고 ㅇㅇ..
			{
				...
				if (MouseOnUI & m_tORTHOUI_INFO.uchSelectUI)
				{
					if (Engine::KeyDown(KeyCheck_Lbutton))
					{
						...
						m_pPlayer->Set_Stat(CUnit::STAT_STATPOINT, (_int)-1);
...
void CStat_UI_Bar::PlayerStatControll()
{
	if (m_tORTHOUI_INFO.uchSelectUI & (MouseClickUI)) //클릭 했다면
	{
		...
		{
		case Engine::STATbar_HP:
			m_pPlayer->Set_Stat(CUnit::STAT_MAXHP, IncreasePlayerMaxHp);
			break;
		case Engine::STATbar_Stamina:
			m_pPlayer->Set_Stat(CUnit::STAT_MAXSTAMINA, IncreasePlayerMaxStamina);
			break;
		case Engine::STATbar_Food:
			m_pPlayer->Set_Stat(CUnit::STAT_MAXHUNGER, IncreasePlayerMaxFood);
			break;
		case Engine::STATbar_Water:
			m_pPlayer->Set_Stat(CUnit::STAT_MAXTHIRST,IncreasePlayerMaxWater);
			break;
		case Engine::STATbar_Weight:
			m_pPlayer->Set_Stat(CUnit::STAT_MAXWEIGHT,  IncreasePlayerMaxWeight);
			break;
		case Engine::STATbar_Atk:
			m_pPlayer->Set_Stat(CUnit::STAT_ATTACK, IncreasePlayerMaxAtk);
			break;
		case Engine::STATbar_Speed:
			m_pPlayer->Set_Stat(CUnit::STAT_SPEED, IncreasePlayerMaxSpeed);
			break;
		case Engine::STATbar_Engram:
			m_pPlayer->Set_Stat(CUnit::STAT_ENGRAM, IncreasePlayerMaxEngram);
			break;
		case Engine::STATbar_Endure:
			m_pPlayer->Set_Stat(CUnit::STAT_ENDURE, IncreasePlayerMaxEndure);
			break;
		case Engine::STATbar_Paralysis:
			break;
		case Engine::STATbar_End:
...
CStat_UI_Bar * CStat_UI_Bar::Create(LPDIRECT3DDEVICE9 _pDevice)
{

	CStat_UI_Bar*	pInstance = new CStat_UI_Bar(_pDevice);
...
CUI * CStat_UI_Bar::Clone(void * _pArg)
{
	CStat_UI_Bar*	pClone = new CStat_UI_Bar(*this);
