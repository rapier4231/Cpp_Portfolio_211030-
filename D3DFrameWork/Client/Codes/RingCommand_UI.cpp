#include "stdafx.h"
#include "..\Headers\RingCommand_UI.h"
...
HRESULT CRingCommand_UI::Set_Screen_UI()
{
...
CRingCommand_UI::CRingCommand_UI(LPDIRECT3DDEVICE9 _pDevice)
	: COrthoUI(_pDevice)
{
}

CRingCommand_UI::CRingCommand_UI(const CRingCommand_UI & rhs)
	: COrthoUI(rhs)
{
}

HRESULT CRingCommand_UI::Ready_GameObject_Prototype()
{
...
HRESULT CRingCommand_UI::Ready_GameObject(void * _pArg/* = nullptr*/)
{
	...
	//Temp.emplace_back(Teemp);
	//static_cast<CRingCommand_UI*>(Engine::Get_GameObject(L"RingCommand_UI"))->Open_RingCommand(Temp);

	return S_OK;
}

_int CRingCommand_UI::Update_GameObject(const _double & _dTimeDelta)
{
...
_int CRingCommand_UI::LateUpdate_GameObject(const _double & _dTimeDelta)
{
	if (!m_tORTHOUI_INFO.bRender)
		return NO_EVENT;
...
void CRingCommand_UI::Render_GameObject(_mat _matOrtho)
{
...
HRESULT CRingCommand_UI::Add_Component(void)
{
	CComponent*		pComponent = nullptr;

	pComponent = m_pRing_Command_Tex = static_cast<CTexture*>(Engine::Clone_ComponentPrototype(L"Proto_Texture_RingCommand"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_DYNAMIC].emplace(L"Com_Texture", pComponent);

	pComponent = m_pShaderCom = static_cast<CShader*>(Engine::Clone_ComponentPrototype(L"Proto_Shader_RingCommand_UI"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
HRESULT CRingCommand_UI::SetUp_ConstantTable(LPD3DXEFFECT & _pEffect, _mat _matOrtho, _int _bTwo)
{
...
HRESULT CRingCommand_UI::SetUp_ConstantTable_To_EffectUI(LPD3DXEFFECT & _pEffect, _mat _matOrtho, _int _bTwo)
{
	_mat matWorld, matView;
	D3DXMatrixIdentity(&matWorld);
...
_int CRingCommand_UI::Mouse_Func(const _double& _dTimeDelta)
{
	_int iChoice = -1;
...
void CRingCommand_UI::Select_Func(_int _iSelectCommand)
{
...
void CRingCommand_UI::Split_To_Circle()
{
	//벡터 공간 확보
...
void CRingCommand_UI::Render_Font()
{
	for (_int i = 0; i < m_iCommandAmount; i++)
	{
		_vec3 vPos;
		...
		Engine::Render_Font(L"Font_TapName", m_vecCommand[i].wstrCommand_Text, &_vec2(vPos.x, vPos.y), RING_FONT_COLOR,2);
	}
}

void CRingCommand_UI::Ef_Func(const _double & _dTimeDelta)
{
	if (!m_tORTHOUI_INFO.bRender)
	{
		m_fRotate[0] = 0.f;
...
CRingCommand_UI * CRingCommand_UI::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CRingCommand_UI*	pInstance = new CRingCommand_UI(_pDevice);
...
CUI * CRingCommand_UI::Clone(void * _pArg)
{
	CRingCommand_UI*	pClone = new CRingCommand_UI(*this);
...
void CRingCommand_UI::Free()
{
	COrthoUI::Free();
}

void CRingCommand_UI::Open_RingCommand(vector<STRCD> _vecCommand, CUnit* pUnit)
{
	m_vecCommand.clear();
...
void CRingCommand_UI::Close_RingCommand()
{
	m_bOpen = false;
	m_pUnit = nullptr;
	m_fOpen = 0.f;
