#include "stdafx.h"
#include "..\Headers\FeedingTrough.h"
...
CFeedingTrough::CFeedingTrough(LPDIRECT3DDEVICE9 _pDevice)
	: CStructure(_pDevice)
{
}

CFeedingTrough::CFeedingTrough(const CFeedingTrough & _rhs)
	: CStructure(_rhs)
{
}

HRESULT CFeedingTrough::Ready_GameObject_Prototype()
{
...
HRESULT CFeedingTrough::Ready_GameObject(void * _pArg)
{
...
_int CFeedingTrough::Update_GameObject(const _double & _dTimeDelta)
{
...
_int CFeedingTrough::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
void CFeedingTrough::Render_GameObject()
{
	CStructure::Render_GameObject();
}

CFeedingTrough * CFeedingTrough::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CFeedingTrough*	pInstance = new CFeedingTrough(_pDevice);
...
CGameObject * CFeedingTrough::Clone(void * _pArg)
{
	CFeedingTrough*	pClone = new CFeedingTrough(*this);
...
HRESULT CFeedingTrough::Add_Component()
{
	CComponent*		pComponent = nullptr;

	/// 매쉬 컴포넌트
	pComponent = m_pMeshCom = static_cast<CStaticMesh*>(Engine::Clone_ComponentPrototype(L"Proto_StaticMesh_Structure_FeedingTrough_Empty"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(L"Com_Mesh", pComponent);

	pComponent = m_pMeshEmpty = static_cast<CStaticMesh*>(Engine::Clone_ComponentPrototype(L"Proto_StaticMesh_Structure_FeedingTrough"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(L"Com_MeshEmpty", pComponent);

	pComponent = m_pMeshLodCom = static_cast<CStaticMesh*>(Engine::Clone_ComponentPrototype(L"Proto_StaticMesh_Structure_FeedingTrough_Lod"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
