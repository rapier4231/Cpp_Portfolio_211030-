...
CCampfire::CCampfire(LPDIRECT3DDEVICE9 _pDevice)
	: CStructure(_pDevice)
{
}

CCampfire::CCampfire(const CCampfire & _rhs)
	: CStructure(_rhs)
{
}

HRESULT CCampfire::Ready_GameObject_Prototype()
{
...
HRESULT CCampfire::Ready_GameObject(void * _pArg)
{
...
_int CCampfire::Update_GameObject(const _double & _dTimeDelta)
{
...
_int CCampfire::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
CCampfire * CCampfire::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CCampfire*	pInstance = new CCampfire(_pDevice);
...
CGameObject * CCampfire::Clone(void * _pArg/* = nullptr*/)
{
	CCampfire*	pClone = new CCampfire(*this);
...
_bool CCampfire::Set_Ignition(_bool _bIsIgnition)
{
	if (_bIsIgnition)
	{
		if (!m_bIsIgnition)
...
HRESULT CCampfire::Add_Component()
{
	...
	/// 매쉬 컴포넌트
	pComponent = m_pMeshCom = static_cast<CStaticMesh*>(Engine::Clone_ComponentPrototype(L"Proto_StaticMesh_Structure_CampFire"));
...
void CCampfire::Check_Roast(const _double & _dTimeDelta)
{
	if (m_bIsIgnition)
	{
