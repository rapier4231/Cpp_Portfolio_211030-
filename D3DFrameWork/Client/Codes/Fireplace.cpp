...
CFireplace::CFireplace(LPDIRECT3DDEVICE9 _pDevice)
	: CStructure(_pDevice)
{
}

CFireplace::CFireplace(const CFireplace & _rhs)
	: CStructure(_rhs)
{
}

HRESULT CFireplace::Ready_GameObject_Prototype()
{
...
HRESULT CFireplace::Ready_GameObject(void * _pArg)
{
...
_int CFireplace::Update_GameObject(const _double & _dTimeDelta)
{
...
_int CFireplace::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
void CFireplace::Render_GameObject()
{
	CStructure::Render_GameObject();
}

CFireplace * CFireplace::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CFireplace*	pInstance = new CFireplace(_pDevice);
...
CGameObject * CFireplace::Clone(void * _pArg)
{
	CFireplace*	pClone = new CFireplace(*this);
...
HRESULT CFireplace::Add_Component()
{
	CComponent*		pComponent = nullptr;

	/// 매쉬 컴포넌트
	pComponent = m_pMeshCom = static_cast<CStaticMesh*>(Engine::Clone_ComponentPrototype(L"Proto_StaticMesh_Structure_Fireplace"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(L"Com_Mesh", pComponent);

	pComponent = m_pMeshLodCom = static_cast<CStaticMesh*>(Engine::Clone_ComponentPrototype(L"Proto_StaticMesh_Structure_Fireplace_Lod"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
