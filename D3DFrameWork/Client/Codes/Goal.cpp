...
CGoal::CGoal(LPDIRECT3DDEVICE9 _pDevice)
...
HRESULT CGoal::Ready_GameObject(void * _pArg/* = nullptr*/)
{
...
_int CGoal::Update_GameObject(const _double & _dTimeDelta)
{
	if (0.f > m_fAlpha)
		Set_Dead();
...
_int CGoal::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
CGoal * CGoal::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CGoal*	pInstance = new CGoal(_pDevice);
...
CGameObject * CGoal::Clone(void * _pArg/* = nullptr*/)
{
	CGoal*	pClone = new CGoal(*this);
...
HRESULT CGoal::Add_Component(void)
{
	...
	pComponent = m_pBufferCom = dynamic_cast<CVIBuffer_GoalParticleTex*>(Engine::Clone_ComponentPrototype(L"Proto_Com_VIBuffer_GoalParticleTex"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(L"Com_Buffer", pComponent);

	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_ComponentPrototype(L"Proto_Texture_Goal"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
HRESULT CGoal::SetUp_ConstantTable(LPD3DXEFFECT & _pEffect)
{
