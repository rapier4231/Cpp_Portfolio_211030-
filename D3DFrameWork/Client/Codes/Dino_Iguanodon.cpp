...
CDino_Iguanodon::CDino_Iguanodon(LPDIRECT3DDEVICE9 _pDevice)
	: CDino(_pDevice)
{
}

CDino_Iguanodon::CDino_Iguanodon(const CDino_Iguanodon & rhs)
	: CDino(rhs)
	, m_iIndex(rhs.m_iIndex)
{
}

HRESULT CDino_Iguanodon::Ready_GameObject_Prototype()
{
...
HRESULT CDino_Iguanodon::Ready_GameObject(void * _pArg)
{
	CDino::Ready_GameObject(_pArg);

	// 이구아노돈 전용 변수
	/////////////////////////////
	m_wstrDinoSpecies = L"Igu";
	m_wstrDinoSpeciesKor = L"이구아노돈";
	/////////////////////////////
	m_iAsddleIndex = 1;								// 매쉬상에서의 안장 서브셋 번호
	m_pSaddleBoneName = "c_back1";	// 안장 뼈 이름
	m_pNeckBoneName = "c_neck3";
	...
	//// 기본 세팅
	m_iAnimation = IGUANODON_ANIM_IDLE;
	...
	m_iUpperAnimation = IGUANODON_ANIM_IDLE;
	...
	Load_Collider(L"../Resources/Data/Collider/Iguanodon_Collider.dat");

	Load_DinoInfo(L"../Resources/Data/DinoInfo/DinoData.ini", L"IGUANODON");
...
_int CDino_Iguanodon::Update_GameObject(const _double & _dTimeDelta)
{

	return CDino::Update_GameObject(_dTimeDelta);
}

_int CDino_Iguanodon::LateUpdate_GameObject(const _double & _dTimeDelta)
{
...
void CDino_Iguanodon::Render_GameObject()
{
	CDino::Render_GameObject();
}

void CDino_Iguanodon::Render_GameObjectShadowMap()
{
	CDino::Render_GameObjectShadowMap();
}

void CDino_Iguanodon::Render_GameObjectShadow()
{
	CDino::Render_GameObjectShadow();
}

_bool CDino_Iguanodon::HitBy(CUnit * pAttacker, _ulong dwAttackType, const _vec3& vCollisionPos)
{
...
CDino_Iguanodon * CDino_Iguanodon::Create(LPDIRECT3DDEVICE9 _pDevice)
{
	CDino_Iguanodon*	pInstance = new CDino_Iguanodon(_pDevice);
...
CGameObject * CDino_Iguanodon::Clone(void * _pArg)
{
	CDino_Iguanodon*	pClone = new CDino_Iguanodon(*this);
...
void CDino_Iguanodon::Free()
{
	CDino::Free();
}


void CDino_Iguanodon::IsRide_Set_SaddleMat()
{
	/*WCHAR str[255] = L"";
...
_bool CDino_Iguanodon::Harvest_Action(wstring & wstrTargetLayer, CGameObject * pLookObject)
{
	if (pLookObject)
	{
...
void CDino_Iguanodon::Check_State()
{
	...
	if (m_ePreUnitState != m_eUnitState)
	{
		...
		case CUnit::STATE_ATTACK:
			m_iAnimation = rand() % 2 ? IGUANODON_ANIM_STOP_ATTACK : IGUANODON_ANIM_STOP_ATTACK2;
			m_bIsAttack = true;
			break;
		...
		case CUnit::STATE_TORPID:			// 기절 시작
			m_iAnimation = IGUANODON_ANIM_TORPID_IN;
			break;
		case CUnit::STATE_TORPID_IDLE:		// 기절중
			m_iAnimation = IGUANODON_ANIM_TORPID_IDLE;
			break;
		case CUnit::STATE_TORPID_TAMED:		// 기절중 일어나기 (테이밍 완료)
			m_iAnimation = IGUANODON_ANIM_TORPID_OUT_TAMED;
			break;
		case CUnit::STATE_TORPID_WILD:		// 기절중 일어나기 (테이밍 실패)
			m_iAnimation = IGUANODON_ANIM_TORPID_OUT_WILD;
			break;
		case CUnit::STATE_TORPID_EAT:		// 기절중 음식먹기
			m_iAnimation = IGUANODON_ANIM_TORPID_EAT;
			break;
		case CUnit::STATE_DEAD:
			m_iAnimation = IGUANODON_ANIM_TORPID_IN;
			break;
		case CUnit::STATE_HARVEST:
			m_iAnimation = IGUANODON_ANIM_HARVEST;
			break;
	...
	if (STATE_MOVE == m_eUnitState || STATE_SPRINT == m_eUnitState)
	{
		...
		case CUnit::UNIT_DIR_DOWN:
			m_iAnimation = IGUANODON_ANIM_BACKWARD;
			break;
		...
		case CUnit::UNIT_DIR_UP:
			if (STATE_MOVE == m_eUnitState)
				m_iAnimation = IGUANODON_ANIM_MOVE_FORWARD;
			else
				m_iAnimation = IGUANODON_ANIM_CHARGE_FORWARD;
			break;
	...
	if (m_ePreUpperDinoState != m_eUpperDinoState)
	{
		...
		case CUnit::STATE_LANDING:
			break;
		case CUnit::STATE_ATTACK:
			m_iUpperAnimation = rand() % 2 ? IGUANODON_ANIM_MOVE_ATTACK : IGUANODON_ANIM_MOVE_ATTACK2;
			m_bIsAttack = true;
			break;
		case CUnit::STATE_HURT:
			break;
		case CUnit::STATE_EAT:
			break;
		case CUnit::STATE_HARVEST:
			m_iUpperAnimation = IGUANODON_ANIM_HARVEST;
			break;
...
HRESULT CDino_Iguanodon::Add_Component()
{
	...
	/// 매쉬 컴포넌트
	pComponent = m_pMeshCom = static_cast<CDynamicMesh*>(Engine::Clone_ComponentPrototype(L"Proto_DynamicMesh_Dino_Iguanodon"));
...
HRESULT CDino_Iguanodon::SetUp_ConstantTable(LPD3DXEFFECT & _pEffect)
{
