#ifndef __VIBUFFER_BLOODPARTICLETEX_H__
#define __VIBUFFER_BLOODPARTICLETEX_H__

#include "VIBuffer.h"

BEGIN(Engine)

class ENGINE_DLL CVIBuffer_BloodParticleTex : public CVIBuffer
{
public:
	typedef struct
	{
	...
	typedef struct
	{
		...
		_float 			m_fImageIndex;		// Image Index
	}PARTICLE;
...
private:
	explicit CVIBuffer_BloodParticleTex(LPDIRECT3DDEVICE9 pGraphicDev);
	explicit CVIBuffer_BloodParticleTex(const CVIBuffer_BloodParticleTex& rhs);
	virtual ~CVIBuffer_BloodParticleTex(void);
...
public:
	static CVIBuffer_BloodParticleTex*	Create(LPDIRECT3DDEVICE9 pGraphicDev);
...
private:
	...
	BOOL				m_bAni;
	_float				m_fTime = 0.f;
	_float				m_fImageWidth = 0.f;
	_float				m_fImageHeight = 0.f;
};

END

#endif // __VIBUFFER_BLOODPARTICLETEX_H__