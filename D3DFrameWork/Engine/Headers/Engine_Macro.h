...
namespace Engine
{
	...
	#define ERR_MSG(_message) MessageBox(NULL, _message, L"System Message", MB_OK)
	...
	#define NULL_CHECK_RETURN_MSG( _ptr, _return, _message )								\
		...
		// FAILED Check
	#define FAILED_CHECK(_hr)	if( ((HRESULT)(_hr)) < 0 )									\
		{MessageBox(NULL, L"Failed", L"System Error",MB_OK);return E_FAIL;}
	#define FAILED_CHECK_RETURN(_hr, _return)	if( ((HRESULT)(_hr)) < 0 )					\
...
//UI Define
#define FNULL 0.f
#define BASICUISIZE 10.f
#define FTRUE 1.f
#define FHALF 0.5f
#define UIAUTOKEY 100
}
